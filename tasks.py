    ...
    with os.scandir(path) as it:
        for entry in it:
            if entry.is_file():
                ...
                result += get_direct_size(entry.path)
    return result
...
def direct_info(direct: Path):
    ...
    rows = []
    with open('dir_info.json', 'w') as f_json, open('dir_info.csv', 'w', newline='', encoding='utf-8') as f_csv,\
            open('dir_info.pickle', 'wb') as f_pickle:
...
if __name__ == '__main__':
    direct_info(Path('C:/Users/User/Desktop/1'))