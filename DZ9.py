...
def deco_csv(function: Callable):
    ...
    def wrapper():
        with open('solv_equ.csv', 'r', encoding='UTF-8') as file:
...
def json_result(func: Callable):
    result = {}
    if os.path.exists('solutionsolved_eq_of_equation.json'):
        with open('solved_eq.json', 'r', encoding='UTF-8') as file:
            result = json.load(file)
    else:
        with open('solved_eq.json', 'w', encoding='UTF-8') as file:
    ...
    def wrapper(*args):
        ...
        return roots
    print("Решение квадратных уравнений  добавлено в файл solved_eq.json")
    return wrapper


def create_coefficients_csvfile():
    with open('solv_equ.csv', 'w', encoding='UTF-8') as file:
