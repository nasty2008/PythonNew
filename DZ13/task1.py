from task2 import MinLengthMistake, TriangleMistake, TestBagSize
...
class Triangle:
    ...
    def __init__(self, a: int, b: int, c: int) -> None:
        if a > 0:
            ...
            raise MinLengthMistake(a, 0)
        if b > 0:
            self.b = b
        else:
            raise MinLengthMistake(b, 0)
        if c > 0:
            self.c = c
        else:
            raise MinLengthMistake(c, 0)
    ...
    def check_sides(self):
        ...
        if (check1 < self.c or check2 < self.b or check3 < self.a):
            raise TriangleMistake(self.a, self.b, self.c)
        else:
    ...
    def check_item(self):
        for elements in self.items:
            if self.bag < self.items[elements]:
                raise TestBagSize(self.bag, self.items[elements])
